...
from browserbase import AsyncSession
...
async def test_get_downloads(
    ...
    faux_file = FauxAsyncIO()

    # Send the downloaded "zip" file to the faux_file buffer
    await async_session.get_zipped_downloads(faux_file)

    assert faux_file.data == b"zipfile-content"


@pytest.mark.asyncio
async def test_get_live_urls(
    async_session: AsyncSession,
    session_id: str,
    respx_mock: MockRouter,
):
    ...
    urls = await async_session.get_live_urls()
...
async def test_get_logs(
    ...
    logs = await async_session.get_logs()
...
async def test_get_recording(
    ...
    recordings = await async_session.get_recording()
